(ns sewerops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (SewerOpsAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the SewerageOpsGovernor (:govern)
...
(defn build
  ...
  `sewerops.store/Store`).
  opts:
    :advisor      -- a `sewerops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; SewerOpsAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; SewerageOpsGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
